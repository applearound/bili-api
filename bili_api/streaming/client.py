...
class DanMuBot:
    GET_DANMU_INFO_URL: str = 'https://api.live.bilibili.com/xlive/web-room/v1/index/getDanmuInfo'
    GET_DANMU_INFO_HEADERS: dict = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36 Edg/87.0.664.66'
    }
    HEARTBEAT_BYTES: bytes = b'[object Object]'
    HEARTBEAT_PACKET: bytes = Header(16 + len(HEARTBEAT_BYTES), 16, 1, 2, 1).to_bytes() + HEARTBEAT_BYTES
    ...
    async def __get_danmu_server_token_and_address(self: DanMuBot) -> tuple[str, str]:
    ...
    async def __listen_to_danmu_server(self: DanMuBot, key: str, danmu_server_address: str):
        async with aiohttp.ClientSession() as session:
            async with session.ws_connect(danmu_server_address) as ws:
                await ws.send_bytes(self.build_login_packet(self.__room_id, key))
